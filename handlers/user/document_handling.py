...
from handlers.user import get_text_from_docx
from handlers.user import get_text_from_pdf
...
async def document_handling(message: Message, bot: Bot) -> None:
    ...
    try:
        if file_path.endswith(".docx"):
            text = await get_text_from_docx.extract_text_from_docx(doc)  # Вызываем функцию для получения текста
        elif file_path.endswith(".pdf"):
            text = await get_text_from_pdf.extract_text_from_pdf(doc)  # Вызываем функцию для получения текста
        else:
